...
print("\n--- Vector Search ---")
# Use a query that matches via substring (since Bedrock has no local creds)
...
# Sort by created_at ASC for chain verification (list_all returns DESC)
...
# 7. Memory reinforce
print("\n--- Memory Operations ---")
# Use the most recently stored memory (from this test run)
recent = mem.list_all()
if recent:
    mid = recent[0].memory_id
    agent = recent[0].agent_id
    print(f"  Reinforcing memory {mid[:8]}... (agent={agent})")
