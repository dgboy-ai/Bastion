...
| A5 | A2A server now has RBAC with 3 roles (reader/writer/admin) + per-key mapping | FIXED |
...
| NEW | a2a_bridge no longer leaks server API key to external A2A servers | FIXED |
| NEW | Guard screening now blocks on failure instead of proceeding unchecked | FIXED |
| NEW | RBAC role resolution now supports per-key mapping via BASTION_A2A_ROLES | FIXED |
| NEW | RLS context failure raises exception in non-mock mode (prevents data leak) | FIXED |
| NEW | docker-compose HMAC secrets use env var references (no hardcoded secrets) | FIXED |
| NEW | A2A streaming guard now uses exc_info=True for proper stack traces | FIXED |
| NEW | Dashboard hourly growth no longer presents fabricated mock data as real | FIXED |
| NEW | A2A brute-force cache now has periodic cleanup in background thread | FIXED |
...
*Generated by deep analysis of `mcp_server.py`, `a2a_server.py`, `memory.py`, `guard.py`, `docker-compose.yml`, dashboard API routes, and tests.*
*Fixes verified: 94 mock tests pass, 56/56 real CockroachDB+Bedrock tests pass.*
*22 gaps fixed total (14 original + 8 new from fresh scan).*
