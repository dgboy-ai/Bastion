...
from bastion.models import MemoryRecord, AuditEntry
...
class TestMemoryDreamer:
    ...
    def test_extract_lesson_with_metadata_hint(self):
        mem = _make_memory(
            content="This is a longer piece of content that exceeds the minimum length for lesson extraction and processing",
            importance=3.0,
    ...
    def test_find_consolidation_candidates_similar(self):
        ...
        self.engine._memories.extend([
            _make_memory(
                content="Python decorators are functions that modify other functions by wrapping them with additional behavior",
                memory_type=EPISODIC,
                importance=6.0,
                created_at=now - timedelta(hours=1),
                memory_id="mem-sim-1",
