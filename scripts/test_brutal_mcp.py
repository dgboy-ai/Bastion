...
# Get the underlying FastMCP app and its tool registry
# We'll test by directly calling the tool functions
...
async def test_store():
    ...
    # Find the tool function in the MCP server
...
async def test_patch():
    ...
    if store_tool and patch_tool:
        ...
        mid = stored_data.get("memory_id")
        if mid:
            result = await patch_tool.fn(ctx, memory_id=mid, patch_ops=[{"op": "replace", "path": "/metadata/tags", "value": ["patched"]}])
            return json.loads(result) if isinstance(result, str) else result
    return {"error": "tools not found"}
...
async def test_ltm_store():
    for tool in mcp._tool_manager._tools.values():
        if tool.name == "ltm_store_analysis":
            ctx = type('MockCtx', (), {'client_id': 'brutal-test'})()
            result = await tool.fn(ctx, query="test analysis query", result="This is a test analysis result", analysis_type="test")
            return json.loads(result) if isinstance(result, str) else result
    return {"error": "tool not found"}
