...
async def main_async() -> None:
    ...
    parser.add_argument("--mock", type=str, default=None, help="Set to 'true' or 'false' to override database mock status")
    ...
    for tool_name, params in benchmarks:
        ...
        results.append(res)
        if not args.json:
            print(f"Tool: {res.name:<18} | Avg: {res.avg_ms:6.2f}ms | p50: {res.p50_ms:6.2f}ms | p95: {res.p95_ms:6.2f}ms | p99: {res.p99_ms:6.2f}ms | rate: {res.success_rate:.0%}")
    ...
    print("| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |")
    for r in results:
        print(f"| `{r.name}` | {r.samples} | {r.avg_ms:.2f} ms | {r.min_ms:.2f} ms | {r.max_ms:.2f} ms | {r.p50_ms:.2f} ms | {r.p90_ms:.2f} ms | {r.p95_ms:.2f} ms | {r.p99_ms:.2f} ms | {r.throughput:.1f} ops/s |")
    print()
