...
from bastion.a2a_server import _TASK_VALID_TRANSITIONS, create_a2a_server
...
test("A2A memory is mock", lambda: memory._mock)
    ...
    resp = client.post(
        ...
        json={
            ...
            "params": {
                "message": {
                    "role": "user",
                    "parts": [{"type": "text", "text": "Store memory: Test A2A"}],
...
def t_get_task():
    resp = client.post(
        "/", json={"jsonrpc": "2.0", "id": 2, "method": "GetTask", "params": {"id": "nonexistent-id"}}, headers=headers
    )
...
def t_invalid_json():
    resp = client.post(
        "/",
        content="not-json",
        headers={"Content-Type": "application/json", "Authorization": f"Bearer {api_key}", "a2a-version": "1.0"},
    )
...
def t_clean_alert():
    ...
    assert result["guard_result"]["is_safe"]
...
def t_poisoning():
    ...
    assert not result["guard_result"]["is_safe"]
...
def t_investigate():
    ...
    assert investigation["time_travel"]["clean_state_found"]
    assert investigation["hash_chain_verification"]["valid"]
