...
def test_core_memory_operations() -> bool:
    ...
    for i in range(20):
        mem.store(
            "fact",
            f"Test memory {i}: Important data about {'security' if i % 3 == 0 else 'performance' if i % 3 == 1 else 'architecture'}",
        )
    ...
    print("         Pin/unpin works correctly")
...
def test_concurrent_operations() -> bool:
    ...
    print("         ✅ 50 concurrent stores completed, all unique IDs")
    ...
    print("         ✅ 20 concurrent searches completed")
    ...
    print("         ✅ Concurrent reads + writes completed safely")
...
def test_mcp_server() -> bool:
    ...
    for _name, tool in server._tool_manager._tools.items():
...
def test_knowledge_graph() -> bool:
    ...
    record, entities, relations = mem.store_with_graph("Alice works at Google and uses Python for machine learning")
...
def test_dreaming() -> bool:
    ...
    mem = BastionMemory("dream-test", mock=True)

    # Store some duplicate-ish memories
    for _i in range(5):
        mem.store("fact", "The server uses CockroachDB for persistence")
        mem.store("fact", "CockroachDB provides SERIALIZABLE isolation")
