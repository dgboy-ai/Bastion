...
def main():
    ...
    cur.execute("SELECT action, COUNT(*) FROM agent_audit WHERE action LIKE '%guard%' OR action LIKE '%block%' OR action LIKE '%security%' OR action LIKE '%inject%' GROUP BY action")
