...
def create_backup(cluster_id: str) -> dict:
    ...
    result = run_ccloud(
        "cluster", "backup", "create", cluster_id,
        "--label", f"bastion-manual-{datetime.now(UTC).strftime('%Y%m%d-%H%M%S')}",
        "-o", "json",
    )
...
def list_backups(cluster_id: str) -> list[dict]:
    ...
    result = run_ccloud(
        "cluster", "backup", "list", cluster_id,
        "-o", "json",
    )
...
def verify_backup(cluster_id: str) -> dict:
    ...
    verify_result = run_ccloud(
        ...
        "--execute", "SELECT count(*) AS table_count FROM information_schema.tables WHERE table_schema = 'public'",
        "-o", "json",
    )
    ...
    if "error" not in verify_result and isinstance(verify_result, list) and len(verify_result) > 0:
        ...
        print(f"    ✓ SQL endpoint accessible")
