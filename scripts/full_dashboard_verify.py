...
def main():
    ...
    print(
        f"  Total Memories:        DB={total_memories}  Dashboard=863  [{'MATCH' if total_memories == 863 else 'MISMATCH'}]"
    )
    ...
    print(
        f"  OWASP Defended:        DB={owasp}  Dashboard=12   [{'MATCH (FIXED)' if owasp == 12 else 'DB=' + str(owasp)}]"
    )
    print()
