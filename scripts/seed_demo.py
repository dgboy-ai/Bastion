...
memories_1 = [
    ...
    (
        "preference",
        "Customer #1042 is on the Enterprise plan ($2,400/mo). Escalate任何 billing issues to account manager.",
    ),
    (
        "instruction",
        "When customer reports 504 errors, first check CockroachDB connection pool metrics before escalating.",
    ),
    ("fact", "Customer #1042's team uses Python SDK v2.3.1 with asyncpg connection pooling (min=2, max=10)."),
    (
        "learned",
        "504 errors on /api/dashboard correlate with connection pool exhaustion during peak hours (9-11 AM EST).",
    ),
    ...
    (
        "instruction",
        "Always include connection pool stats in 504 error investigation reports for Enterprise customers.",
    ),
    ("fact", "Customer #1042's last support ticket (#SR-8842) was resolved in 2.3 hours — above their 4-hour SLA."),
    (
        "learned",
        "Customer #1042's 504 errors increased 300% after their last deployment on July 8. Correlation with new middleware.",
    ),
    ("fact", "Customer #1042 has 15 team members with CockroachDB access. Admin: Sarah Chen, DevOps: Mike Park."),
    (
        "preference",
        "Customer #1042 prefers technical responses with SQL queries and metrics, not generic troubleshooting steps.",
    ),
    ...
    (
        "learned",
        "Customer #1042's connection pool exhaustion is caused by long-running transactions holding connections.",
    ),
    ("fact", "Customer #1042 has a custom dashboard showing real-time CockroachDB metrics (Grafana + Prometheus)."),
    (
        "preference",
        "Customer #1042 prefers async communication. Response within 24 hours is acceptable for non-urgent.",
    ),
    ...
    (
        "instruction",
        "When sharing best practices, reference their existing architecture to make recommendations actionable.",
    ),
...
memories_2 = [
    ...
    (
        "learned",
        "AS OF SYSTEM TIME requires follower reads or strong reads. Use follower for latency, strong for consistency.",
    ),
    ...
    (
        "instruction",
        "Dreaming runs during idle periods. Consolidate duplicates, promote high-value episodic to semantic.",
    ),
    ...
    (
        "learned",
        "Router selects strategy based on query type: vector for semantic, BM25 for keyword, graph for relational.",
    ),
...
memories_3 = [
    ...
    (
        "episodic",
        "July 10, 2026: Customer #1042 reported 504 errors on /api/dashboard. Investigated connection pool exhaustion. Resolved by increasing max_connections from 50 to 100.",
    ),
    (
        "episodic",
        "July 12, 2026: Code review for PR #4525 — added circuit breaker for Bedrock API. Caught missing retry logic before merge. 3 iterations needed.",
    ),
    (
        "episodic",
        "July 14, 2026: Deployed Bastion v0.6.0 to production. 22 memories stored, hash chain verified. All 10/10 features confirmed working.",
    ),
    ...
    (
        "semantic",
        "SERIALIZABLE isolation is mandatory for agentic workloads. READ COMMITTED causes cascading errors when agents compound bad data.",
    ),
    ...
    (
        "procedural",
        "When debugging 504 errors: 1) Check connection pool metrics 2) Verify statement_timeout 3) Run EXPLAIN ANALYZE 4) Check for full table scans 5) Escalate if >4 hours.",
    ),
    (
        "procedural",
        "PII scan workflow: Detect email/phone/SSN/CC → Redact in-place → Log detection type → Compute hash on redacted content → Store with PII flag in metadata.",
    ),
    (
        "procedural",
        "Memory consolidation (dreaming) process: Fetch recent memories → Find duplicates via Jaccard similarity → Merge duplicates → Promote high-value episodic to semantic → Prune expired.",
    ),
    # === SECURITY MEMORIES (threat intelligence) ===
    (
        "security",
        "OWASP ASI06: Memory poisoning is the #3 risk for agentic systems. Mitigation: SHA-256 hash chains + content validation + trust scoring.",
    ),
    (
        "security",
        "Prompt injection patterns blocked: 'ignore previous instructions', 'admin override', 'system prompt', 'you are now', 'disregard all'. 9 regex patterns + LLM classifier.",
    ),
    (
        "security",
        "Secret detection patterns: AWS keys (AKIA...), GitHub tokens (ghp_...), private keys (-----BEGIN), generic API keys. All blocked before storage.",
    ),
    ...
    (
        "fact",
        "CockroachDB's multi-region capabilities: automatic data placement, leaseholder preferences, zone constraints.",
    ),
...
entities = [
    ...
    (
        ...
        "pull_request",
        "PR #4526",
        '{"type": "feature", "description": "time-travel queries"}',
    ),
