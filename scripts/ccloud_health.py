...
def check_cluster_health(cluster_id: str) -> dict:
    ...
    latency_result = run_ccloud(
        ...
        cluster_id,
        "--execute",
        "SELECT now() AS server_time, gateway_id FROM crdb_internal.gateway_nodes LIMIT 1",
        "-o",
        "json",
    )
    if "error" not in latency_result:
        print("    ✓ SQL endpoint responsive")
    else:
    ...
    storage_result = run_ccloud(
        ...
        cluster_id,
        "--execute",
        "SELECT sum(range_size_bytes) AS total_bytes FROM crdb_internal.ranges_no_local",
        "-o",
        "json",
    )
    ...
    count_result = run_ccloud(
        ...
        cluster_id,
        "--execute",
        "SELECT count(*) AS total_memories FROM agent_memory",
        "-o",
        "json",
    )
    if "error" not in count_result and isinstance(count_result, list) and len(count_result) > 0:
        ...
        print("    Could not retrieve memory count (table may not exist yet)")
    ...
    audit_result = run_ccloud(
        ...
        cluster_id,
        "--execute",
        "SELECT count(*) AS total_audit_entries FROM agent_audit",
        "-o",
        "json",
    )
