...
def insert_memory(cur, agent_id, mtype, content, prev_hash, importance=7.0, trust=3, days_ago=0):
    ...
    cur.execute(
        ...
        (mid, agent_id, mtype, content, json.dumps(emb), ch, prev_hash, importance, trust,
         "seed_script", created),
    )
    return ch, mid
...
memories_3 = [
    ...
    ("procedural", "Knowledge graph: Extract triples → Store entities/relations → BFS traversal → Time-travel snapshot."),
...
memories_4 = [
    ...
    ("fact", "CDC changefeeds stream writes to Lambda for real-time anomaly detection."),
    ("fact", "Lambda CDC handler verifies hash chain, detects corruption, triggers self-healing."),
...
memories_5 = [
    ...
    ("fact", "CDC pipeline: changefeed → Lambda → S3 + SNS alerts."),
    ("fact", "CDC latency: < 5 seconds end-to-end."),
    ("learned", "CDC failed once on July 18 — Lambda timeout increased to 60s."),
    ("instruction", "Monitor CDC lag. Alert if > 30 seconds."),
