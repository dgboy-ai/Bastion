...
run_sql("agent_keys previous_encrypted_dek column",
    "ALTER TABLE agent_keys ADD COLUMN IF NOT EXISTS previous_encrypted_dek BYTES")

run_sql("push_notification_log table", """
...
run_sql("push_notification_log pending index",
...
run_sql("push_notification_log task index",
...
run_sql("agent_memory isolation policy", """
...
run_sql("agent_keys isolation policy", """
...
run_sql("agent_entities isolation policy", """
...
run_sql("agent_relations isolation policy", """
...
run_sql("a2a_tasks isolation policy", """
...
run_sql("thought_graph isolation policy", """
...
run_sql("cdc_agent_memory changefeed", """
...
run_sql("cdc_agent_audit changefeed", """
...
run_sql("cdc_a2a_tasks changefeed", """
...
for name, rls, force in rls_tables:
...
for t in ["agent_keys", "push_notification_log"]:
    cur.execute("SELECT count(*) FROM information_schema.tables WHERE table_name = %s AND table_schema = 'public'", (t,))
