...
test(
    ...
    blocked >= total_attacks * 0.7,
    f"{blocked}/{total_attacks} ({blocked / total_attacks * 100:.0f}%)",
)
...
test(
    "Analyst detects poisoning",
    r_poison["guard_result"]["findings"] is not None
    and len(r_poison["guard_result"]["findings"]) > 0
    or not r_poison["guard_result"]["is_safe"],
...
if r_poison["escalated"]:
    ...
    test(
        "Responder time-travel",
        inv["time_travel"]["clean_state_found"],
        f"found={inv['time_travel']['clean_state_found']}",
    )
    test("Responder heals memory", inv["healing"]["healed_memory_id"] is not None)
    test(
        "Responder verifies chain",
        inv["hash_chain_verification"]["valid"],
        f"valid={inv['hash_chain_verification']['valid']}",
    )
...
if FAIL > 0:
    ...
    if blocked < total_attacks:
        print(
            f"    - Guard detection rate: {blocked}/{total_attacks} ({blocked / total_attacks * 100:.0f}%) — needs more patterns"
        )
