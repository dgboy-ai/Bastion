  ...
  return (
    <div style={{ background: "#0a0508", minHeight: "100vh", position: "relative", overflow: "hidden", color: "#e8e8ed" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "20px 32px", maxWidth: "1400px", margin: "0 auto" }}>
        ...
        {/* ── 2-COLUMN GRID SYSTEM (Fills page perfectly side-by-side, no massive empty voids) ── */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.1fr", gap: "24px", alignItems: "start" }}>
          
          {/* LEFT COLUMN: Welcome Hero or Active Demo steps */}
          <div>
            {isWelcome ? (
              <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", minHeight: "560px", paddingRight: "10px", animation: "fadeUp 0.5s ease-out" }}>
                
                {/* welcome badge */}
                <div style={{ display: "inline-flex", alignSelf: "flex-start", alignItems: "center", gap: "6px", padding: "6px 14px", borderRadius: "999px", background: "rgba(255,94,0,0.07)", border: "1px solid rgba(255,94,0,0.2)", marginBottom: "18px" }}>
                  <span style={{ fontSize: "9px", color: "#ff9100" }}>✦</span>
                  <span style={{ fontSize: "9px", color: "#ff9100", fontWeight: 800, letterSpacing: "1.5px", textTransform: "uppercase" as const }}>interactive gateway</span>
                </div>

                {/* Hero title */}
                <h1 style={{ fontSize: "42px", fontWeight: 850, margin: "0 0 16px 0", lineHeight: "1.15", letterSpacing: "-1.5px", color: "#fff" }}>
                  Verify your AI agent&apos;s <br />
                  <span style={{ background: "linear-gradient(135deg, #ff9100, #ff5e00, #ff2a00)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Memory Integrity</span>
                </h1>

                {/* Subtitle */}
                <p style={{ fontSize: "15px", color: "#a0a0b0", margin: "0 0 28px 0", lineHeight: "1.6", maxWidth: "480px" }}>
                  Run live scenarios verifying secure database actions. Test memory injections, trace crypto hash chain links, and recover states via MVCC time-travel query functions.
                </p>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "32px" }}>
                  <button onClick={() => goStep(1)} style={{
                    ...
                    color: "#fff", fontWeight: 700, fontSize: "14.5px", cursor: "pointer",
                    boxShadow: "0 0 30px rgba(255,94,0,0.2)",
                    display: "inline-flex", alignItems: "center", gap: "8px",
                    transition: "transform 0.2s"
                  }}>
                    Start Demo Tour <span style={{ fontSize: "14px" }}>▶</span>
                  </button>
                  <span style={{ fontSize: "12px", color: "#606070" }}>3 simple steps · 90s</span>
                </div>

                {/* 3 highlight columns */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", borderTop: "1px solid #1a1a2a", paddingTop: "24px" }}>
                  {[
                    { icon: "🛡️", label: "Injection Detection", desc: "Blocks poison payloads instantly." },
                    { icon: "⏱️", label: "MVCC Time Travel", desc: "Queries CockroachDB pre-attack state." },
                    { icon: "🔗", label: "Hash Chain Audit", desc: "Cryptographically guarantees audit." }
                  ].map((x, i) => (
                    <div key={i} style={{ background: "rgba(255,255,255,0.015)", border: "1px solid #1a1a2a", borderRadius: "10px", padding: "12px" }}>
                      <div style={{ fontSize: "18px", marginBottom: "4px" }}>{x.icon}</div>
                      <div style={{ fontSize: "12px", fontWeight: 700, color: "#fff", marginBottom: "3px" }}>{x.label}</div>
                      <div style={{ fontSize: "10.5px", color: "#606070", lineHeight: "1.4" }}>{x.desc}</div>
                    </div>
                  ))}
                </div>
              </div>
